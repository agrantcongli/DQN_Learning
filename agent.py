...
class Replaymemory:
    ...
    def sample(self):
        available_num = min(self.memo_cntr, self.MEMORY_SIZE)
        idxs = np.random.choice(available_num, self.BATCH_SIZE)
        print(idxs)
        batch_s = self.all_s[idxs]
        batch_a = self.all_a[idxs]
        batch_r = self.all_r[idxs]
        batch_done = self.all_done[idxs]
        batch_s_ = self.all_s_[idxs]
        # print(batch_a.shape)
        # print(batch_a)
...
class Agent:
    def __init__(self, n_input, n_output):
        ...
        self.online_net = DQN(n_input, n_output) # TODO
        self.target_net = DQN(n_input, n_output) # TODO
